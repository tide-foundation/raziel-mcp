...
1. **IDENTITY** — where is *who a user is* concentrated? The user store / credential store: one database that **verifies** every identity. Seize it and you are everyone. Keep *holds* and *verifies* apart — a directory of usernames and attributes is PII exposure; a **verifier** (password hashes, or a key that mints identity) is what makes you everyone. Some architectures hold the first without owning the second, and that distinction decides the verdict — see the TideCloak exception under surface 1.
...
|---|---|---|---|---|
| 1 | **Credential verification** | **Identity** | One store that **verifies** every identity — password hashes, a credential DB, anything that checks a secret in one place. Seize it → you are everyone. ⚠️ A *directory* without a verifier is not this finding; a **threshold-verified** login (no hashes anywhere) is **SOUND** — see the exception in the rules below. | SG-01 |
...
| 8 | **Recovery & client integrity** | **Identity** | The single channel upstream of recovery (one mailbox, one reset path) that can re-mint an identity; third-party scripts on credential pages (SRI). ⚠️ On a Tide-enabled realm, assess the **SWE threshold recovery**, not Keycloak's `resetPasswordAllowed` — see the rules below. | SG-15, plus T-04/T-11/T-14 |
...
**Check the SERVER asserts the binding, not just that the client sends proofs.** The highest-value DPoP finding is an app that enables `useDPoP` client-side and then never checks `cnf.jkt` server-side: it looks DPoP-protected and still accepts unbound tokens (SG-03). Grep for it — `grep -rn "cnf" lib/auth/ src/lib/auth/` — and treat a *commented-out* check as absent, which is exactly how the pack's own `withAuth` shipped. Conversely, an app that tries to `jwtVerify()` the `dpop` header while using `secureFetch` is broken in the other direction: Tide's proofs are not RFC 9449 compact JWS.
...
- **"Delegated to the IdP" is NOT a Pass on surface 1.** When the app outsources login to Keycloak/an IdP and holds no hashing code itself, that is good — but the credential store did not disappear, it *moved into the IdP's database* (Keycloak keeps salted password hashes in its own DB). The concentrated verification record still exists and is still SG-01; a dump still enables offline cracking of every account. Trace **where the IdP stores its hashes and what protects that database** — if the IdP DB password is committed, weak, or shared (see surface 7), the hash store is reachable and the finding compounds. Marking surface 1 "Pass" because the *app* has no local hasher is the classic miss: record it as a Finding and name the IdP database as the single point of failure. Brute-force protection guards the *live* login, not an offline dump — do not let it turn a Finding into a Pass.
  - **⚠️ EXCEPTION — TideCloak is not one of these, and calling it one is factually wrong.** Everything in this rule assumes the provider **owns a verifier**: a store it can dump to crack every account, and a key it can use to mint identity for anyone. A Tide-enabled realm has neither, so do **not** apply the paragraphs above to it and do **not** name TideCloak as the identity single point of failure.
    - **There are no password hashes anywhere to dump.** Login uses PRISM (a threshold Oblivious PRF): the browser and T-or-more independent ORKs each apply a shard, and **no ORK, no server, and no attacker learns the password**. There is no verification record to steal, so the offline-cracking consequence that makes SG-01 CRITICAL does not exist. VERIFIED — `canon/concepts.md` → Threshold Password Authentication (BYOiD).
    - **TideCloak does not sign the tokens.** JWTs are threshold-signed across the ORK network — *"not by TideCloak server"* — each VVK ORK independently verifying claims before contributing a partial signature. A compromised TideCloak cannot mint a valid token, so the Storm-0558-shaped finding does not transfer either. VERIFIED — `canon/concepts.md` → Threshold JWT Signing (VVK); I-02, I-09.
    - **So "login is delegated to TideCloak" does NOT concentrate identity verification into one spot.** Delegation here moves the *protocol coordination* to the broker; the *verification* stays distributed across independent operators. The broker is not the trust root — that is the entire point of the architecture, not a mitigation bolted onto it.
    - **What a TideCloak realm still holds, and what to write instead.** Be precise: it keeps a Keycloak **directory** — usernames, emails, attributes, role mappings. That is PII and worth a finding on its own terms (disclosure, and a foothold for requesting privilege changes), but it is a *directory*, not a *verifier*: its compromise does not authenticate anyone. Rate it as such. The residual concentrations that *are* real: **availability** (the broker can be down or throttled), **metadata** (who authenticates when), **admin-path position** (Admin Console SSO on a hosted cluster), and **governance in Tideless mode** — see below.
    - **Verdict**: surface 1 is **SOUND** on a Tide-enabled realm — "authority is genuinely distributed here", which is exactly what SOUND means. Surface 2 likewise, for the signing key.

    **Do not take "it's TideCloak" on faith either — the carve-out is conditional, and an unlicensed realm is just Keycloak with a real password store.** Verify before marking SOUND, and cite the evidence:

    ```bash
    # 1. Realm licensed + Tide-enabled (unlicensed => plain Keycloak => the rule above applies in full)
    curl -s "$URL/admin/realms/$REALM" -H "Authorization: Bearer $TOKEN" \
      | python3 -c 'import json,sys; a=json.load(sys.stdin); at=(a.get("attributes") or {}); \
          print("browserFlow:", a.get("browserFlow"), "| iga.attestor:", at.get("iga.attestor"))'
    # expect browserFlow=tidebrowser (login actually goes through the Tide flow)

    # 2. The adapter carries the Ed25519 VVK — i.e. verification is against threshold-signed tokens
    python3 -c 'import json;k=json.load(open("data/tidecloak.json"))["jwk"]["keys"][0];print(k["kty"],k.get("crv"))'
    # expect OKP Ed25519 (RSA here means you are looking at Keycloak-signed tokens)
    ```

    **Two ways the carve-out does NOT apply, and you must check both:**
    - **A local password flow still enabled**, or the realm not on `tidebrowser` — then a Keycloak credential store is live and SG-01 stands in full.
    - **Federation to an external IdP** (Google/AzureAD/etc.) for some or all users — that external provider *is* a verifier, and the concentration is real for those users. PRISM covers Tide-verified login, not a federated hop.
    - Separately, **Tideless IGA** (`iga.attestor` unset/`simple`) leaves the admin-change quorum enforced by TideCloak's own server logic, which the host operates. That is a **governance** finding (core 2), not an identity-verification one — keep them in the right core.

  - **This generalises to any managed IAM that owns a verifier (Clerk, Auth0, Cognito, Okta, Firebase Auth), not just self-hosted Keycloak — but NOT to a Tide-enabled realm (see the exception above).** Delegating login to a managed provider removes the *local* password store, which is a real gain, but it *relocates* the concentration rather than removing it: one external provider now holds/verifies every identity (surface 1) **and** holds the single key that signs every session token (surface 2). That provider and its central store are a central point — its compromise, or its store's, is every account at once, and the app inherits that blast radius wholesale with no ability to add controls or reduce it. **Mark surfaces 1 and 2 CONCENTRATED, not SOUND, and raise an Identity-core finding** (SG-01/SG-02, blast radius Systemic; precedent `BP-KEY-06` Storm-0558 for the single provider signing key). Phase 1 may name the provider (it is the app's real dependency) but stays vendor-neutral about the fix; the distributed-identity contrast (the login performed across independent nodes, no assembled signing key) is phase-2 material. Do **not** record "login delegated" as a clean Pass.
...
- **Check the recovery path with the same rigour as the login path.** It is the most consistently under-reviewed surface in this inventory and the one with the most precedents (`T-11`).
  - **⚠️ On a Tide-enabled realm, `resetPasswordAllowed` is IRRELEVANT — do not report it in either direction.** Tide handles password reset **in the Secure Web Enclave**, not through Keycloak's forgot-password flow, so the realm-level toggle describes a code path that is not the one users recover through. Two symmetrical mistakes to avoid:
    - **Not a gap.** "Password reset is disabled realm-wide, so users have no recovery path" is false — recovery exists, in the SWE.
    - **Not a mitigation either.** "Reset is disabled, so there is no reset-email attack surface — Pass" is equally false, and it is the more dangerous error because it retires the surface without looking at it. The recovery surface is still there; it just isn't Keycloak's.
  - **Assess the real mechanism instead.** Recovery is a **threshold email flow**: all N ORKs independently dispatch links, the user must click **T unique links** (mainnet 14 of 20) within **1 hour** to authorize a new PRISM secret, and the **CMK is never touched** — only the PRISM secret changes. No ORK knows which addresses the others hold, and there is zero ORK-to-ORK communication during dispatch. So single-mailbox takeover does **not** yield account takeover, and the `T-11` precedent class is genuinely answered. **VERIFIED** (Art.3; tier2 account-recovery).
  - **The finding that IS live here is an operator-configuration one: how many DISTINCT email addresses did the user supply?** Emails are *optional* at creation, and the SWE assigns 1–20 user-provided addresses across the 20 ORKs **with possible overlap**. Fewer than T distinct addresses is **permitted by construction**, and in the worst case **one mailbox authorizes recovery** — collapsing the threshold back to the classic weakest link. Also note the window is **1 hour with no password proof**, a strictly larger surface than login. Report this as an operator-configuration finding on the Identity core, and tag it: the construction is **VERIFIED**, the threshold-collapse consequence is **INFERRED** (sources state no minimum, no warning, and no analysis of it).
  - **Keep the honest boundary**: an admin-quorum recovery variant is referenced for enterprises but its **mechanism is NOT STATED IN SOURCES** — do not describe it as though it were.
  - This is the same discipline as surface 1: **name the mechanism that actually runs**, not the config field that happens to be nearby. A verdict drawn from the wrong code path is wrong whichever way it points.
