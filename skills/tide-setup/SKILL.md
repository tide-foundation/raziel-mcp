...
- **Version is load-bearing, and you must discover it — never hardcode one.** Run `templates/shared/skycloak-latest-version.sh` to get the newest TideCloak tag at or above the verified `0.14.17` floor. Older versions provision happily and then fail later at licensing or at the automation client. `latest` is not a value Skycloak accepts, and tags must be sorted numerically (`"0.9.8" > "0.14.20"` as strings).
