...
- **Version is load-bearing. Discover it from SKYCLOAK — never hardcode one, and never read it off Docker Hub.** Skycloak exact-matches against a server-side allowlist, so the newest published tag is often not provisionable (`400 invalid cluster version`). Ask `GET /clusters/supported-versions?type=tidecloak`, or run `templates/shared/skycloak-latest-version.sh`, which does that and applies the verified `0.14.17` floor. Older versions provision happily and then fail later at licensing or on the automation client. `latest` is not a value Skycloak accepts, and versions must be sorted numerically (`"0.9.8" > "0.14.20"` as strings). If Skycloak's newest is old, that is server-side — report it (AP-84).
