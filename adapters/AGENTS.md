...
- **Role/claim mappers**: use **stock Keycloak mapper types only**. `tide-roles-mapper` **does not exist** — MEASURED on the production image, `/admin/serverinfo` lists 24 openid-connect providers and none contains "tide". A realm.json declaring it imports with **201 Created** and the mapper is **silently dropped**, so roles go missing with no error anywhere. Roles come from the default `roles` client scope (`oidc-usermodel-realm-role-mapper` / `oidc-usermodel-client-role-mapper`); Tide's own claims come from the `tide-claims` scope that `setUpTideRealm` provisions (`tideuserkey`, `vuid`, `t\.uho` — all stock types). AP-80; see `canon/tidecloak-bootstrap.md`.
- **Canonical setup ordering**: License (`setUpTideRealm`) → IGA (`toggle-iga`) → E2EE. Only valid sequence. See `canon/tidecloak-bootstrap.md`.
- **Docker image**: always `tideorg/tidecloak-dev:latest` — despite the `-dev` suffix this IS the production image and the only supported one. No ORK/threshold/payer env vars needed (built-in defaults) and none should be supplied. Never use `tidecloak-stg-dev`.
...
- **Version policy**: Pin all `@tidecloak/*` packages to the CURRENT latest — resolve with `npm view <pkg> version` at build time; fall back to `0.14.17` if npm is unreachable. Never 0.99.x, never below 0.14.17. Match the SDK version to the TideCloak server version. See `canon/version-policy.md`.
