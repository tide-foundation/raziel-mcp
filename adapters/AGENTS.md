...
- **Role mappers**: include standard Keycloak role mappers (`realm roles`, `client roles`) in realm templates so token role claims survive regardless of image. (An earlier claim that a `tide-roles-mapper` provider exists only on the production image is UNVERIFIED — see `canon/tidecloak-bootstrap.md`.)
